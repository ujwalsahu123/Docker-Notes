...
2. [React Project with Docker](#react-project-with-docker)
3. [Dockerfile Full Template](#dockerfile-full-template)
4. [Docker Images](#docker-images)
5. [Docker Containers](#docker-containers)
6. [docker run Command Variations](#docker-run-cmd-variations)
7. [Docker Image Behavior Experiment (Retag)](#xtra-docker-image-behavior-experiment-retag)
8. [Pre-Defined Images](#pre-defined-images)
9. [Docker Image Versioning and Container Deployment](#docker-image-versioning-and-container-deployment)
10. [General Docker Commands](#general-docker-cmds)
11. [Docker Hub](#docker-hub)
12. [Docker Volume](#docker-volume)
13. [Docker Bind Mounts](#docker-bind-mounts)
14. [.dockerignore](#dockerignore)
15. [Communication Between Containers / Systems](#communication)
...
If we share only the code with another person, they may install different versions of dependencies or run different commands. Because of these differences, the application may fail to run.

This is the common problem called: “Works on my machine”.
...
You can think of it as an end-to-end packaged version of the application folder. Anyone who gets this image does not need to manually install software, dependencies, or configure anything.
...
The container behaves like a small isolated computer inside your system. It contains everything that was inside the image:
- project files
...
<p align="center"><img src="Notes/docker_diagram.png" width="600"></p>

<p align="center"><img src="Notes/docker_desktop.png" width="600"></p>
...
we do this before RUN , since npm install requires package.json file , requirements.txt needed before doing **`docker RUN pip install -r requirements.txt`**
...
(so it's NOT like the above where you rebuild with the same name:tag and the images disappear; here images don't disappear)
...
then again it will give the same project:latest name . and thus the Image Tag Reuse will happen and then old image will exist but disappear.
...
When a container runs an application (example: React app using `npm start`), it runs inside the container environment.  
Even if the app is running on port 3000 inside the container, we still cannot access it from our browser.
...
`docker run -p 3000:3000 image_id`  
or  
`docker run -p 3123:3000 image_id`  
or  
`docker run -p 3069:3000 image_id`
...
so container_port can be the same for multiple containers, but the Host_port must be different. 
...
To make a Container with an interactive terminal (basically we can take input from user in the terminal)  
...
-------[OPTIONS]--------  
You can write multiple Options & There is NO Precedence.  
...
- (write the cmd just like you write in terminal, with space separated)

ex:  
**`docker run image_name npm run dev`**  
**`docker run --name container1 image_name python file.py`**
...
And the older images just lose their name:tag and become <none>:<none> and disappear , but still exist.
...
When container1 and container2 were deleted, then the old reference images are also deleted.  
and disk becomes empty (this tells that those images were also removed as we removed those containers)
...
**`docker image ls`**  -> you can see the image   
...
We don't give a new image name for each deploy version; instead we use Tag_name  
...
(don't make after adding a feature or etc, only when you want to deploy or share)

NOTE:  
so images you can think like this -> you made a basic website then made a image and deployed,  
and then updated some stuff then again made a new image and then deployed the new version, and removed the previous deployed version.  
or you can also think like -> you made different versions of your website and deployed them all at the same time,  
to different users/locations/etc (ex:- 200 different versions of Facebook run in parallel .. to test different features and stuff)
...
stop docker engine -> quit Docker Desktop  /or/ wsl --shutdown (this will shut down WSL; open Docker Desktop to start the Docker engine again)
...
Important rule:  
So basically in a dockerhub repo all images must have -> the same image_name but tag_name can be different.  
...
we can clone any image from Docker Hub, and those images are stored in our Docker Desktop application folder on the C: drive; using the CLI or Desktop app we can access those images and make containers with them.
...
// like Docker Hub, there are other registries also like AWS-ECR (Elastic Container Registry), Google-GCR, Azure-ACR  
...
Docker containers are isolated and temporary. If a container stores data inside its own filesystem, that data exists only inside that container.  
If the container is deleted, the data stored inside it is also deleted.
...
A Docker Volume is a persistent storage location managed by Docker. Instead of storing data inside the container, the container stores data in the volume.  
The volume exists outside the container, so even if the container is deleted, the data remains safe.
...
(Also, only the data part is stored in the volume, not things like code files or application files. Basically only the user-generated or runtime data is stored there.  
Docker handles this automatically — we do not have to explicitly specify that "store data.txt" or any particular file.)  
...
so while coding and testing ... we need to repeatedly test the code and data to check if it works on this data or not.  
so in that case you have to build new images and run them repeatedly to test if the new code and data is working.  
...
so basically the container directly uses the folders and files from our local repo on the host machine.  
so that if we change any code or data in our repo files then that change will also be reflected in the container ... so  
no need to repeatedly create images and run ... we just simply make a bind mount container ... and do testing using just a single container ...  
...
container path → the folder in which the files you want to get updated 

if entire repo you want to bind then:  
repo_folder:working_dir_folder  -> ex ->  C:\names_app:/app

if particular files only you want to bind mount then:  
...
we can also just bind a particular file -> ex -> -v C:\react_proj\service.txt:/app/service.txt (so only the service.txt file will be bound/connected)

### Concept

so basically all the files inside the container actually comes from C:\myapp\  
and if you update the code in local repo program.js then that same code will be used by the container and immediately will run it.  
...
if we delete the container then it doesn't delete the local repo project  
container → deleted  
...
The updates are Immediately Run in Container.  
...
we can make multiple bind mount containers and all use the local repo files and folders... so if we update the local repo files ... or if we update any container repo files ... then it will be reflected in the local repo as well as all the container repos which were connected using bind mount.
...
(so this same behaviour was also in the volumes where we were able to use the same data for different containers using the volume, but here its not just data but also other files and folders, and they are bind mounted/ connected with host repo also.)
...
put it inside the local repo folder
repo/  
│  
├─ testapp/  
│   ├─ Dockerfile  
...
<p align="center"><img src="Notes/communication1.png" width="600"></p>
...
<p align="center"><img src="Notes/communication2.png" width="600"></p>
...
Example when writing SQL queries :-  
instead of this -> **`host = "localhost"`**  
write -> **`host = "host.docker.internal"`**    // and then build the image and run the container

<p align="center"><img src="Notes/communication3.png" width="600"></p>
...
**`docker inspect mysqldb`**  -> inspect this container, and you can see the Network:ip_address:172.32.234.3  -> so at this IP the SQL container is running.
...
`docker run -v mysql-data:/var/lib/mysql image`
...
Example:  
**`docker network create my-net`**  
**`docker run mysql_container`**  
**`docker run app_container`**  
**`docker run redis_container`**  
**`docker run nginx_container`**  
**`docker run -d -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE="userinfo" --name mysqldb mysql`**
...
Both containers automatically connect to the same network, and there's no need to write it separately.  
(in some cases when we run docker-compose up then it also builds the image .. and then runs it. so then if we do **`docker-compose down`** then it removes the container but doesn't delete that built image.  
and next time when we do docker-compose up then it uses that built image)
...
When an application gets more users, one container may not be enough to handle all requests. Instead of increasing the power of a single container, we usually run multiple containers of the same application.
...
All containers run the same image (same code, same environment). The load balancer distributes incoming requests between them so that no single container gets overloaded.
...
and if traffic increases then you can set -> Replicas required: 10 or more
...
RUN npm install                  # install dependencies (to run terminal cmd to make image , pip install requirement.txt , etc ..)
...
**`docker rm container_id/name`**    # delete container
...
Rename the selected image to push as per the dockerhub repo name:  
...
you can now delete the local image and container from your machine:  
**`docker rm container`**  
**`docker rmi image`**  
